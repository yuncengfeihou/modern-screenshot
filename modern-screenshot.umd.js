(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.modernScreenshot = {}));
...
`);
  ...
  async function cloneIframe(iframe, context) {
    try {
      const contentDoc = iframe.contentDocument;
      if (contentDoc?.documentElement) {
        const clonedHtml = await cloneNode(contentDoc.documentElement, context);
        if (isHTMLElementNode(clonedHtml)) {
          const computedStyle = window.getComputedStyle(iframe);
          const style = clonedHtml.style;
          style.margin = "0";
          style.padding = "0";
      ...
      context.log.warn("Failed to clone iframe content", error);
    }
...
//# sourceMappingURL=modern-screenshot.umd.js.map
